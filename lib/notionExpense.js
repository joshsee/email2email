...
function getDatabaseId(key) {
    ...
    const databaseId = envKey && process.env[envKey];

    if (!databaseId) {
        throw new NotionExpenseError(`Missing ${envKey} environment variable`, 500);
    }
...
function resolveWalletLookup(transaction) {
    ...
    if (transaction.format === 'directdebit') {
        return { lookup: 'name', value: CASH_WALLET_NAME };
    }
