...
function parseDirectDebitFormat(text) {
    if (!/Direct Debit/i.test(text)) {
        return null;
    }

    const dateMatch = text.match(/Transaction Date:\s*(\d{4})\/(\d{2})\/(\d{2})/i);
    const payeeMatch = text.match(/^Payee:\s*(.+)$/im);
    const amountMatch = text.match(/Amount:\s*([A-Z]{3})\s+([\d,]+\.?\d*)/i);
    const referenceMatch = text.match(/Debtor's Reference:\s*(\S+)/i);

    if (!dateMatch || !payeeMatch || !amountMatch) {
        throw new TransactionParseError('Failed to parse BOC Direct Debit transaction email');
    }
    ...
    return {
        ...
        merchant: payeeMatch[1].trim().split('\n')[0].trim(),
        currency: currency === 'CNH' ? 'CNY' : currency,
        amount: normalizeAmount(amountMatch[2]),
        referenceNo: referenceMatch ? referenceMatch[1] : null,
        format: 'directdebit',
...
function parseBocTransaction(text) {
    ...
    const directDebit = parseDirectDebitFormat(normalized);
    if (directDebit) {
        return directDebit;
    }
